...
dynamixel_sdk::PacketHandler *Driver::GetPacketHandler() {
  ...
  return dynamixel_sdk::PacketHandler::getPacketHandler(
      configuration_.protocol_version);
}
...
dynamixel_sdk::PortHandler *Driver::GetPortHandler() {
  ...
  return dynamixel_sdk::PortHandler::getPortHandler(
      configuration_.device_name.c_str());
}
...
std::vector<ServoSpeed> Driver::GetPresentSpeeds(std::vector<int> &servo_ids) {
  ...
  for (std::vector<int>::iterator i = servo_ids.begin(); i != servo_ids.end();
       ++i) {
    servo_id = *i;
    ...
    result = packet_handler_->read2ByteTxRx(
        port_handler_, servo_id, configuration_.control_table.moving_speed,
        &present_speed, &error);
    ...
    if (result != configuration_.control_values.success) {
      std::printf(
          ...
          servo_id_string.c_str(), packet_handler_->getTxRxResult(result),
          result);
      throw error;
    } else if (error != 0) {
      if (configuration_.debug == true) {
  ...
  if (rpm == 0) {
    ...
    int value =
        static_cast<int>(std::nearbyint(
            rpm / configuration_.control_values.moving_speed_rpm_unit)) +
...
void Driver::SetMovingSpeeds(std::vector<ServoSpeed> &speeds) {
  ...
  for (std::vector<ServoSpeed>::iterator i = speeds.begin(); i != speeds.end();
       ++i) {
    ...
    if (result != configuration_.control_values.success) {
      ...
      std::printf(
          "Error setting moving speed for servo ID %s to %i (%f rpm): %s\n",
          servo_id_string.c_str(), speed, rpm,
          packet_handler_->getRxPacketError(error));
      throw error;
    } else {
...
void Driver::SetTorqueLimit(std::vector<int> &servo_ids, int limit) {
  ...
  for (std::vector<int>::iterator i = servo_ids.begin(); i != servo_ids.end();
    ...
    result = packet_handler_->write2ByteTxRx(
        port_handler_, servo_id, configuration_.control_table.torque_limit,
        limit, &error);
...
double Driver::SpeedToRpm(int speed) {
  if (speed > 0 &&
      speed <= configuration_.control_values.moving_speed_cw_maximum) {  // CW
    return static_cast<double>(speed) * configuration_.control_values.moving_speed_rpm_unit;
             ...
             speed <= configuration_.control_values
                          .moving_speed_ccw_maximum) {  // CCW
...
void Driver::ToggleTorque(std::vector<int> &servo_ids, bool enabled) {
  uint8_t error;
  int result;
  int torque_enable_value = enabled
                                ? configuration_.control_values.torque_enable
                                : configuration_.control_values.torque_disable;
  ...
  for (std::vector<int>::iterator i = servo_ids.begin(); i != servo_ids.end();
    ...
    if (configuration_.debug == true) {
      std::printf(
          ...
          configuration_.control_table.torque_enable, torque_enable_value);
    }

    result = packet_handler_->write1ByteTxRx(
        port_handler_, servo_id, configuration_.control_table.torque_enable,
    ...
    if (result != configuration_.control_values.success) {
      std::printf(
          ...
          "torque.\n",
          servo_id_string.c_str(), packet_handler_->getTxRxResult(result),
          result, enabled ? "enabling" : "disabling");
      throw error;
    } else if (error != 0) {
